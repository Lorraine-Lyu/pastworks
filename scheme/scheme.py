...
class Frame:
    ...
    def rebind(self, symbol, value):
        ...
        if symbol in self.bindings:
            ...
            Frame.rebind(self.parent, symbol, value)
            return
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals != nil:
            if vals == nil:
                raise SchemeError("formals and values not of the same length!")
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        # print("aaaaaaaaaaa", self.formals)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        # print("aaaaaaaaaa", expressions)
        env.define(expressions.first, scheme_eval(expressions.second.first, env))
        return expressions.first
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        env.define(expressions.first.first,\
            LambdaProcedure(expressions.first.second, expressions.second, env))

...
class Ant(Insect):
    ...
    food_cost = 0
    blocks_path = True  #problem 7
    is_container = False #problem 9
    incremented = False #problem 13
    # ADD CLASS ATTRIBUTES HERE
    ...
    def nearest_bee(self, hive):
        ...
        def find_nearest(place):
            ...
            if random_or_none(place.bees) == None:
                return find_nearest(place.entrance)
            return random_or_none(place.bees)
        return find_nearest(self.place)
        # END Problem 3 and 4
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        def find_nearest(place, curr):
            if place == hive or curr > self.max_range:
                return None
            if random_or_none(place.bees) == None:
                return find_nearest(place.entrance, curr + 1)
            return random_or_none(place.bees)
        return find_nearest(self.place,0)
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        def find_nearest(place, curr):
            if place == hive:
                return None
            if curr < self.min_range or random_or_none(place.bees) == None:
                return find_nearest(place.entrance,curr + 1)
            return random_or_none(place.bees)
        return find_nearest(self.place,0)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.is_imposter:
            ...
            while curr_place:
                if curr_place.ant and not curr_place.ant.incremented:
                    curr_place.ant.incremented = True
                    curr_place.ant.damage *= 2
                if curr_place.ant and curr_place.ant.is_container \
                        and curr_place.ant.contained_ant\
                        and not curr_place.ant.contained_ant.incremented:
                    curr_place.ant.contained_ant.incremented = True

...
public class Router {
    ...
    public static List<Long> shortestPath(GraphDB g,
        ...
        GraphDB.Node des = g.nodes.get(g.closest(destlon, destlat));

        List<Long> toReturn = new ArrayList<>();
        toReturn.add(Long.parseLong(start.id));
        PriorityQueue<Router.Entry> waitlist =
                new PriorityQueue<Router.Entry>(g.nodes.size(), ((o1, o2) -> (o1.value.compareTo(o2.value))));

        for (String s : start.to) {
            Entry toBeAdded = new Entry(s, g.distance(Long.parseLong(start.id), Long.parseLong(g.nodes.get(s).id)));
            waitlist.add(toBeAdded);
        }

        while (!toReturn.get(toReturn.size() - 1).equals(des.id)) {
            String temp = waitlist.poll().id;
            toReturn.add(Long.parseLong(temp));

            for (String s : g.nodes.get(temp).to) {
                Entry toBeAdded = new Entry(s, g.distance(Long.parseLong(g.nodes.get(temp).id), Long.parseLong(g.nodes.get(s).id)) +
                        g.distance(Long.parseLong(g.nodes.get(s).id), Long.parseLong(g.nodes.get(des).id)));
               /** we need a self-defined PQ here, can you add your previous lab assignment here?
                * I'm not sure how exactly update() works. Here I assume PQ contains an Entry and
                * it can find Entries with duplicate keys when adding. In the update method,
                * if the object doesn't exist, we just add it to the PQ*/
                //waitlist.update(toBeAdded);
